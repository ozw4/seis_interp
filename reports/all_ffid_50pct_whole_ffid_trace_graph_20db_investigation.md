...
- 実施日: 2026-08-31 〜 2026-09-01(UTC)
- 状態: `running` — 全 19 の予定 stage が完了。厳密 20 dB 基準は未達のため、
  継続計画(§7)とともに study は open のまま。
...
| 19 | 形式: w128 / r12 / batch 4 @10,000 updates | 12.239 | 未達(< 20) |
| 21 | **形式: w128 / r6 / batch 4 @25,000 updates(study 最良)** | **12.469** | 未達(< 20) |
...
- stage21 の予算カーブ(batch 4、25k コサイン): 2,500→9.910、5,000→10.931、
  7,500→11.409、10,000→11.708、12,500→11.891、15,000→12.130、17,500→12.292、
  20,000→12.382、22,500→12.441、25,000→**12.469**(train audit 12.793、
  19.9 時間、1.43M パラメータ)。
- スケールした GNN は最強コントロール(stage02 の 11.318)を **+1.15 dB** 上回る。
  終盤の増分は +0.09 → +0.06 → +0.03 dB/2,500 と明瞭に漸近しており、
  このモデル族・分割での漸近値は **≈12.5–13 dB** と推定される。
...
データスケーリング(終盤増分 +0.03 dB/2,500 updates まで漸近)の下で、
予算のみの延長で 20 dB に到達する evidence-backed な経路は存在しない。
本 study は study 020 の前例に従い、閾値到達までの継続を前提に `running` の
まま残す。
