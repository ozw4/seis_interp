...
**shear 0・omega 30・time scale 12 の対照は target SNR −3.1486 dB、RMSE 14.2809 で、shear なしの10 dB目標は未達である。** 現在は、最新のユーザー指示に従い shear 0.0006 s/m を明記した **11.3422 dB、RMSE 2.6929** の SIREN 参照モデルを採用する。[座標比較レポート](c3_siren_coordinate_investigation_20260909.md)と [現在の採用判断](../results/study_031_c3_siren_10db/20260909T055026000000Z_e39df16d563c_shear_reference_adoption/adoption_decision.json)に補助変換付きであることを記録している。

比較では omega 30・time scale 12 を保ち、shear だけを 0 にすると精度が悪化した。この設定・同じ seed・5,000 更新で固定 shear が有効だったという結果は変わらない。ただし、観測だけから係数を求めることによる漏洩回避と、他手法との同一前処理は別である。採用時に固定 shear を明記し、GNNを含む他手法へ shear を導入することもしない。
...
| --- | ---: | ---: | ---: | ---: | ---: |
| **shear 0.0006（補助変換付き採用参照）** | **2.3861** | **2.6929** | **11.3422** | **1,179.5277** | **1,216.6931** |
| shear 0（対照） | 5.2413 | 14.2809 | −3.1486 | 1,112.7451 | 1,140.3644 |
...
図は元の出力を保持しており、図中の reference は shear 付き参照条件を指す。左は per-trace 正規化後の MSE の 100 更新区間平均、中央は観測と target の物理 RMSE、右は正解エネルギーで割った target の誤差成分である。shear 0 では観測への適合も target 精度も悪化した。[学習曲線の数値 CSV](../results/study_031_c3_siren_10db/20260909T031103662283Z_1819109f28e1_shear_ablation_comparison/training_loss.csv)。
...
shear 0 の元 run は `20260909T030400486453Z_1819109f28e1_siren`。compact JSON・CSV・PNG は診断成果として保持し、重みと大配列も元の `runs/` から変更しない。[診断 manifest](../results/study_031_c3_siren_10db/20260909T031103662283Z_1819109f28e1_shear_ablation_comparison/manifest.json)、11.3422 dB の [旧採用 manifest](../results/study_031_c3_siren_10db/20260909T023636913643Z_1819109f28e1_final_coordinate_comparison/manifest.json)、[採用撤回の分類記録](../results/study_031_c3_siren_10db/20260909T033540186900Z_1819109f28e1_comparison_contract_revision/comparison_classification.json)は履歴として維持する。最新の [参照モデル採用記録](../results/study_031_c3_siren_10db/20260909T055026000000Z_e39df16d563c_shear_reference_adoption/adoption_decision.json)が現在の判断を定める。採用は固定 shear を明記した SIREN 参照に限り、shear なしの達成は主張しない。本文の小数は 4 桁、機械可読値は丸めず保持している。
