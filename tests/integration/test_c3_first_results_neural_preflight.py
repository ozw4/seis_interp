...
@pytest.mark.parametrize("action", ["nersi", "ccnet-train"])
def test_replaced_preflight_is_rejected_before_input_access_or_writes(tmp_path, action):
    with pytest.raises(ValueError, match="unsupported neural preflight action"):
        run_c3_first_results_neural_preflight(
            action,
            config_path=tmp_path / "missing.yaml",
            suite_dir=tmp_path / "missing-suite",
