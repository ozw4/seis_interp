...
Can one SIREN trained across every amplitude-eligible FFID in SEG C3 Narrow-Azimuth reconstruct
validation traces that are completely held out within each eligible FFID?

This is survey-wide trace interpolation with every amplitude-eligible FFID represented during
training. It is not shot interpolation: no eligible complete FFID is held out, and FFID is not
...
one optimizer update consumes one eligible FFID, and one shared coordinate network is updated
across the quality-filtered survey.
...
Before splitting, raw interim amplitudes are checked trace by trace. A trace is excluded in full
when all 625 samples are exactly zero or any sample has absolute amplitude greater than `1.0e4`;
samples are never clipped or removed individually. This excludes 544 traces, all from FFID 1746:
107 all-zero traces and 437 excessive-amplitude traces. FFID 1746 consequently has no eligible
traces and is absent from training and evaluation. Sail-line-end FFIDs with fewer than the 544
traces of a complete shot otherwise remain part of the dataset. The measured source checksums are
locked in `inputs.yaml`.

## Split and leakage contract

Eligible whole traces are split independently inside every eligible FFID with seed 42. A 0.20
holdout is divided with 0.25 of the holdout assigned to validation and the remainder to test. The
split is based on sorted `array_row` identifiers and an FFID-specific random stream, so filtering
FFID 1746 cannot change another FFID's membership. Every eligible FFID must have non-empty train,
validation, and test sets. Excluded traces retain an explicit `excluded` label in the processed
split table. Time samples from one trace never cross split boundaries.

Only eligible training traces fit coordinate bounds and the global amplitude RMS. Eligible
validation traces select the checkpoint; test and excluded traces do not enter optimization,
early stopping, or study selection.
...
One optimizer update is all eligible training traces of one FFID multiplied by all time samples.
Batch size therefore varies for incomplete FFIDs. One epoch visits every eligible training FFID
exactly once in a seeded shuffled order. The 4,780 eligible FFIDs produce 4,780 optimizer updates
per epoch and a ten-epoch maximum of 47,800 updates. This budget was chosen only to approximate
Study 014's 50,000-update budget; it is not claimed to be optimal. Early-stopping patience is
three epochs.

## Validation and checkpoint selection

At each epoch end, every eligible validation trace is evaluated. Coordinates, targets, and
predictions are materialized for at most one FFID at a time, while reference and error energies
are accumulated into the point-weighted global S/N. The best checkpoint is selected only by this
streaming global validation S/N. No per-FFID or test-set metric affects selection.
...
- Every eligible FFID has train, validation, and test traces without time-sample leakage.
- Every epoch visits each eligible training FFID once, with one complete-FFID optimizer update per
  visit.
...
sweep capacity or learning rate, schedule the learning rate, or use multiple GPUs. The absolute
amplitude threshold is specific to the observed gap in this locked SEG C3 NA input and is not a
general seismic QC threshold. Point-weighted global S/N may hide FFID-level variation. Ten epochs
provide a provisional update budget rather than evidence of convergence.
