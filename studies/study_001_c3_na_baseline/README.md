...
- 2026-08-25: PR4 samples training traces and time indices uniformly with replacement and defines one epoch as `steps_per_epoch` Adam updates with the configured loss. The paper motivates Adam, the loss choice, and early stopping; the POC-specific choices are the sampling details, epoch length, validation batch size, device, selection of the best model by global S/N over every point of the whole validation traces, and patience-based stopping. Only the best checkpoint is saved, including the SIREN constructor arguments, weights, and normalization. Resume state, optimizer state, and test-split evaluation remain outside PR4.
- 2026-08-25: This clean-synthetic interpolation study now trains with `training.loss: l2` at `training.learning_rate: 1.0e-4`. The first full L1 run at `1.0e-5` (`runs/study_001_c3_na_baseline/20260825T053620Z_09d01f2_baseline`) stopped early at epoch 54 with a best validation S/N of -0.004 dB, so the reconstruction was no better than a zero trace. L2 penalizes the large residuals of the dominant events more strongly on noise-free data, and the ten-times-larger step follows from the same run reducing its training loss only from 0.212 to 0.157 over 27000 updates. `training.loss` accepts `l1` and `l2`; Huber stays unimplemented until the noise experiment needs it, where L2, Huber (paper threshold 0.1), and L1 will be compared for denoising and erratic-noise robustness. The epoch history key is `train_loss` instead of `train_l1`, and earlier runs keep their original key as an immutable record. The split and normalization conditions are unchanged, so `data/processed/c3_na/ffid_2348_random_split` is reused and training still verifies that the resolved config matches the preparation contract.
- 2026-08-25: Global validation S/N over all points was dominated by one high-energy validation trace, so it measured that trace rather than the held-out set. Later runs select the best checkpoint and count early-stopping patience by the median of the per-trace validation S/N values. Global validation S/N is still calculated and recorded as a diagnostic. This is a POC evaluation decision, not a paper-specified protocol; earlier runs keep their original metric as an immutable record and are not rewritten.
